...
function confirmDelete() {
    ...
    router.delete(route('branch.account.destroy', userToDelete.value.id), {
        preserveScroll: true,
        onSuccess: () => {
    ...
    <CardTable>
        <template #header-content>
            ...
            <div class="flex justify-between items-center">
                ...
                <Link :href="route('branch.account.create')" class="btn btn-sm btn-primary">
                Add Account
                </Link>
            </div>
        </template>
        ...
        <template #table-content>
            <DataTable :headers="['Name', 'User Code', 'Email', 'Role', 'Branch', 'Status', 'Actions']"
                ...
                <template #row="{ row: user }">
                    ...
                    <td class="px-4 py-2">
                        <Link :href="route('branch.account.edit', user.id)" class="btn btn-primary btn-sm">Edit</Link>
